...
{{"intent":"price|analysis|deep_analysis|comparison|simple|chat|trade_plan|trade_sim|dual_analysis","tickers":["btcusdt"]}}.
...
- "dual_analysis": запрос анализа по нескольким таймфреймам, "двойной анализ", "скальп и свинг", "краткосрок и среднесрок", "multi timeframe", "dual timeframe"
- "chat": все остальное
...
def _classify_regex_fallback(message: str) -> dict | None:
    ...
    # dual_analysis
    if re.search(r'(двойн|скальп.*свинг|свинг.*скальп|multi.*time|dual.*time|краткосрок.*среднесрок|несколько таймфреймов)', msg):
        return {"intent": "dual_analysis", "tickers": []}
