...
только просмотр. `OLLAMA_CONTEXT_LENGTH=262144` из конфига приложения (замер
2026-09-22 в `%LOCALAPPDATA%\Ollama\server.log`) для этой модели нерабочий.
...
Замер 2026-09-22: у процесса приложения (PID 21524, старт 03:48:26) в логе стоит
`OLLAMA_MODELS:D:\WORED`, и в ту же минуту в репозитории создан пустой
`D:\WORED\manifests` (0 файлов) — то есть каталог репозитория был принят за хранилище
моделей. Следствие: `11434/api/tags → {"models":[]}`, отсутствие модели в UI приложения
и `Error: pull model manifest: file does not exist` при любой попытке pull. Значение
нигде не сохранено (HKCU `OLLAMA_MODELS` корректен, HKLM пуст, в реестре ключей Ollama
нет, в репозитории присваивания `D:\WORED` нет) — оно пришло по наследству от
терминала, из которого запускали `ollama app.exe`. Лечится чистым перезапуском
приложения из меню «Пуск», а не из терминала; пустые `D:\WORED\blobs` и
`D:\WORED\manifests` — следы этого эпизода, их удаление требует отдельного решения.
