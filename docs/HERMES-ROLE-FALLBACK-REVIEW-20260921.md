...
`ROLE_ATTEMPT_BUDGET_SECONDS = 100`. **M4 и M6 (P3) исправлены** (коммит `f5ffabf`):
хвост цепочки на NVIDIA больше не добавляется молча — tier стал опциональным и по
умолчанию выключен (`NVIDIA_NIM_ENABLED`, см. `docs/ENV-REGISTRY.md`), а роль не
начинает с модели, которой в этом же бандле уже ответил другой роль: совпадающие
кандидаты переносятся в конец цепочки, а не удаляются, и дублирование пишется в
лог. Открытыми остаются M5, M7.
