...
начинает с модели, которой в этом бандле уже ответил другой роль: совпадающие
кандидаты переносятся в конец цепочки, а не удаляются, и дублирование пишется в
лог. **M7 (P4) исправлен** (коммит `241e15b`, продолжение `b00351d`): очередь больше не затирает
колонку качества — `execution_state=COALESCE(execution_state,'completed')`, поэтому
`partial`, который runner пишет при провале роли, доживает до статуса; `/api/trader/forecast`
считает полосу по ролям (один голос на `step × роль`, новый прогон перекрывает
повтор того же роли) и отдаёт `coverage` + `execution_state`; в деке статическая
пилюля «демо-модель» заменена живым бейджем покрытия ролей; `partial` стал
терминальным состоянием в поллере. Попутно снят захардкоженный `sigma: 0.008`,
который интерфейс подписывал как «σ часа (EWMA)», и `float(None)` на пустом
`confidence`, из-за которого карточка прогноза падала в `unavailable`. Тем же
прод-замером вскрылось, что 340 из 855 точек имеют NULL фитиль, и `float(value)`
в агрегации превращал молчание роли в ноль на графике — `_as_wick` откатывает
такой фитиль к цене самого голоса (`b00351d`); в деке (`app.py`) та же строка
`float(cr["predicted_low"] or 0)` осталась и описана ниже как часть M8.
**M5 не выполнен намеренно** — это решение владельца про `.env`/`AGENTS.md`, без
вашего слова я его не применяю. Открытой осталась новая находка M8.
...
### M8 — На главной прогноз перестаёт быть виден в момент, когда он становится готовым

Найдено 2026-09-22 при реализации M7. `api_command_deck` (`app.py:3806-3808`)
берёт запрос с `WHERE status='active'` и обязательной будущей точкой
(`p.target_time > NOW()`), тогда как очередь по успешном завершении пишет
`status='completed'`. Замер в прод-БД: из 46 запросов со `status='active'` будущих
точек нет ни у одного; с будущими точками живёт 1 запрос, и он `completed`.
Практически `consensus` на `/` существует только пока роль считается, и исчезает
в тот момент, когда прогноз готов. Вторым слоем там же: `roles` выбираются с
`fmr.status = 'completed'`, поэтому `agreement = N/N` считает только выживших —
та же природа обмана, что и в M7, но на другой странице. Плюс
`float(cr["predicted_low"] or 0)` превращает NULL фитиля (340 из 855 точек) в ноль
на графике — в карточке трейдера это снято правкой `b00351d`, на `/` нет.

**Что с этим делать — не решено:** надо определить, какой прогноз считается
витриной (`latest completed AND valid_until > NOW()` против `status='active'`), и
что делать с нулевыми фитилями. Правка не начата, чтобы не менять смысл главной
страницы без подтверждения владельца.
...
# как агрегируется полоса на реальных строках (после правки M7)
# дамп: scripts/probe_forecast_role_coverage.sql, сам прогон ничего не пишет
python scripts/probe_forecast_role_coverage.py 137 139 141
```
...
Исполнение: P1 → `92233a4`, P2 → `c2fe3bf`, P3 → `f5ffabf`, P4 (M7) → `241e15b`
и `b00351d`.
M5 из P4 не выполнен — ждёт решения владельца по `.env`/`AGENTS.md`. Следующий
кандидат — M8 (видимость готового прогноза на `/`), затем X-5 (бюджет роли против
бюджета задачи).
